...
class MangaNatoScraper implements SourceScraper {
  name = 'MangaNato';
  ...
  async getPopularManga(): Promise<ScraperResult[]> {
    try {
      ...
      console.log(`MangaNato: Found ${results.length} manga from popular page`);
      return results.slice(0, 50);
    } catch (error) {
      console.error('Error fetching from MangaNato:', error);
      return [];
    }
  }
  ...
  async getMangaDetails(id: string): Promise<ScraperResult> {
    try {
      ...
      console.error('Error fetching manga details from MangaNato:', error);
      throw error;
    }
  }
}

export default new MangaNatoScraper();
